...
#ifndef _ANNLAB_STOP_WATCH_LIST_H_
#define _ANNLAB_STOP_WATCH_LIST_H_
...
public:
    ...
    stop_watch_list() : bIsRunning(false), startTime(0), stopTime(0), elapsedTime(0), elapsedTimeTotal(0) {};
    stop_watch_list(const stop_watch_list &src);

    stop_watch_list &operator =(const stop_watch_list &t);
...
inline stop_watch_list::stop_watch_list(const stop_watch_list &src)
{
...
inline stop_watch_list &stop_watch_list::operator =(const stop_watch_list &t)
{
...
inline void stop_watch_list::native_start()
{
#if _WIN32 || _WIN64
...
inline void stop_watch_list::native_stop()
{
#if _WIN32 || _WIN64
    LARGE_INTEGER qp_cnt;
    QueryPerformanceCounter(&qp_cnt);
...
inline stop_watch_list::timestamp_t stop_watch_list::native_now(void)
{
    timestamp_t result;
...
inline double stop_watch_list::native_nowf(void)
{
    double result;
...
inline bool stop_watch_list::isRunning(void)
{
    return bIsRunning;
}

inline void stop_watch_list::clear(void)
{
    elapsedTime = 0;
    elapsedTimeTotal = 0;
...
inline void stop_watch_list::reset(void)
{
    elapsedTime = 0;
...
//! restart() is equivalent to reset() and begin()
inline void stop_watch_list::restart(void)
{
    elapsedTime = 0;
...
inline void stop_watch_list::rebegin(void)
{
    restart();
}

inline void stop_watch_list::start(void)
{
    native_start();
...
inline void stop_watch_list::begin(void)
{
    start();
}

inline void stop_watch_list::stop(void)
{
    native_stop();
...
inline void stop_watch_list::end(void)
{
    stop();
}

inline stop_watch_list::timestamp_t stop_watch_list::now(void)
{
    return native_now();
}

inline double stop_watch_list::nowf(void)
{
    return native_nowf();
}

inline double stop_watch_list::intervalSeconds(int64_t t1, int64_t t2)
{
    double seconds = (double)(t2 - t1) * 1E-9;
    return seconds;
}

inline double stop_watch_list::intervalSeconds(double t1, double t2)
{
    double seconds = (double)(t2 - t1);
    return seconds;
}

inline stop_watch_list::timestamp_t stop_watch_list::currentTimeMillis(void)
{
    timestamp_t now_usecs = stop_watch_list::native_now();
    return now_usecs / static_cast<timestamp_t>(1000UL);
}

inline double stop_watch_list::currentTimeMillisf(void)
{
    double now_usecs = stop_watch_list::native_nowf();
    return now_usecs * 1E-3;
}

inline double stop_watch_list::getSeconds(void)
{
    if (bIsRunning)
        stop();
...
inline double stop_watch_list::getMillisec(void)
{
    if (bIsRunning)
        stop();
...
inline double stop_watch_list::getTotalSeconds(void)
{
    if (bIsRunning)
        stop();
...
inline double stop_watch_list::getTotalMillisec(void)
{
    if (bIsRunning)
        stop();
...
inline double stop_watch_list::getUsedTime(void)
{
    return getSeconds();
}

inline double stop_watch_list::getTotalUsedTime(void)
{
...
#endif  /* _ANNLAB_STOP_WATCH_LIST_H_ */
